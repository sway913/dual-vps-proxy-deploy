...
- 安装 Xray 后、生成正式配置前，分别在两台服务器本机执行 `scripts/plan_reality_targets.py ingress` 与 `scripts/plan_reality_targets.py exit`，使用实测排名第一且可自动采用的 Reality 目标。
...
3. Reality 目标必须由目标服务器本机实测选择，不能因为某个域名曾在另一台机器可用就直接复用。若没有候选通过硬性检查，扩大候选池后重新规划，不得跳过验证。
